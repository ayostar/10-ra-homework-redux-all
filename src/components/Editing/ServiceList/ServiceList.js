...
import {
  removeService,
  changeServiceField,
  changeEditableServiceFields,
  deleteEditableServiceFields,
        ...
        .map((item) => (
          <li className="service-list__item" key={item.id}>
            ...
            <div className="btn__box">
              <button className="btn btn__edit" onClick={handleEdit(item.id)}>
                <span className="_hidden">Редактировать</span>
              </button>
              <button
                className="btn btn__remove"
                onClick={handleRemove(item.id)}
              >
                <span className="_hidden">Удалить</span>
              </button>
            </div>
          </li>
        ))}
